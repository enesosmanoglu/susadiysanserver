...
// The privacy option must be one of ["Private","Unlisted","Public"]. (Default is "Unlisted")
const video2 = { path: 'video2.mp4', title: 'title 2', description: 'description 2', privacy: "Public", thumbnail:'thumbnail.png', language: 'english', tags: ['video', 'github'], playlist: 'playlist name', onSuccess:onVideoUploadSuccess }
...
let uploadURL = fs.existsSync('upload.url') ? fs.readFileSync('upload.url', "utf8") : 'https://www.youtube.com/upload';
    ...
    videos.forEach(video => {
        ...
        if (video.thumbnail) {
            if (video.thumbnail.startsWith('http')) {
                downloadAsync(video.thumbnail, video.path.split('.').slice(0, -1).join('.') + '.png').then(() => {
                    console.log('Thumbnail downloaded');
                })
            } else {
                if (!existsSync(video.thumbnail)) {
...
async function login(localPage, credentials) {
    ...
    try {
        await localPage.waitForSelector(".button.text-button.black-secondary", { timeout: 5000 });
    ...
    try {
        ...
        await localPage.waitForXPath(selectBtnXPath, { timeout: 5000 });
...
async function uploadVideo(videoJSON) {
    ...
    for (let i = 0; i < 2; i++) {
        try {
            ...
            console.log('failed to find the select files button for chapter ', i, nextText);
            console.error(error);
            await page.evaluate(() => { window.onbeforeunload = null });
            await page.goto(uploadURL);
        }
    ...
    if (thumb) {
        ...
        if (thumb.startsWith('http'))
            thumb = pathToFile.split('.').slice(0, -1).join('.') + '.png';
        await thumbChooser.accept([thumb]);
        console.log("Thumbnail selected:", thumb);
    }
    ...
    //await page.goto(await page.url() + "&udvid=" + videoUrl.split('/').slice(-1)[0], { waitUntil: ["networkidle0", "domcontentloaded"] });
    ...
    if (addCartPlaylist) {
        try {
            ...
            try {
                ...
                timeStr = timeToStr(time, timeStr.split(':').length);
                console.log(timeStr);
                await page.focus(`ytcp-media-timestamp-input input`);
                await page.keyboard.down('Control');
                await page.keyboard.press('A');
